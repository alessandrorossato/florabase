...
def _references(connection: Connection) -> dict[str, UUID]:
    ...
    connection.execute(
        text(
            ...
            "VALUES (:id, 'Retired supplier', 'nursery', :now, :now)"
        ),
    ...
    connection.execute(
        text(
            ...
            "(id, name, parent_id, place_kind, retired_at, created_at, updated_at) "
            "VALUES (:id, 'Old provenance', :parent_id, 'custom', :now, :now, :now)"
        ),
        {"id": values["place"], "parent_id": world_id, "now": now},
    )
    connection.execute(
        text(
...
def test_plant_migration_upgrade_downgrade_and_reupgrade(database_engine: Engine) -> None:
    ...
    try:
        ...
        for table in ("plants", "plant_groups"):
            ...
            check_names = {item["name"] for item in inspector.get_check_constraints(table)}
            assert {
                f"ck_{table}_label",
                f"ck_{table}_origin",
                f"ck_{table}_direct_origin_detail",
                f"ck_{table}_collection_entry_date",
                f"ck_{table}_lifecycle",
                f"ck_{table}_notes",
            } <= check_names
            if table == "plant_groups":
                assert "ck_plant_groups_quantity" in check_names
        command.downgrade(config, "20260831_0010")
        inspector = inspect(database_engine)
        assert not inspector.has_table("plants")
        assert not inspector.has_table("plant_groups")
...
def test_plant_database_invariants_and_restrictive_references(
    ...
    for table, item_id in (
        ...
        ("sowings", refs["sowing"]),
        ("suppliers", refs["supplier"]),
        ("geographic_places", refs["place"]),
    ...
    database_connection.execute(
        text("DELETE FROM plant_groups WHERE id=:id"), {"id": retained_group}
    )
